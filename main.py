...
async def full_pipeline(request: QueryRequest):
    ...
    if user_msg_stripped in EXAMPLE_QUERIES_SQL:
        ...
        # We assume hardcoded queries are safe, but you can still run it through validation
        # is_safe, safety_reason = validate_sql_safety(generated_sql) # Optional
    else:
        # ── Step 1: Parse intent ───────────────────────────────────
        ...
        if intent_resp.clarification_needed:
            # Provide common time-range quick-reply options
    ...
    try:
        final_answer, answer_usage = await call_gemini(
